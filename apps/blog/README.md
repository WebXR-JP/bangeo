...
- `NEXT_PUBLIC_BANGEO_SITE_ANALYTICS_KEY`: 通常のサイト計測用公開キー。本番ビルドに設定し、`https://bangeo.net` または `https://www.bangeo.net` からのアクセス時だけSDKを読み込みます。受信先は `https://bangeo-ingest.peraperapera.workers.dev` です。Analytics側でも両方のオリジンを許可してください。
