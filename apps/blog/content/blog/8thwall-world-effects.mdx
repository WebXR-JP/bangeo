...
tags:
  ...
  - "SLAM"
author: "BANGEO Team"
thumbnail: "/assets/tech/8thwall-world-effects.webp"
draft: false
---

## World Effectsとは

8th Wallの**World Effects**は、world tracking（SLAM）を活用し、現実の地面や空間に仮想オブジェクトを配置するAR機能です。world tracking（SLAM）は8th Wallのワールドトラッキング機能で、カメラ映像と3Dシーンをcompositingします。`xrweb`コンポーネントの`defaultEnvironment`系プロパティで空と地面を構成し、カメラ映像と3Dシーンを統合します。
...
World EffectsはSLAM（world tracking）に依存するため、**OSS版エンジンだけでは動作しません**。配布バイナリ（[8thwall/engine](https://github.com/8thwall/engine)の`xr-standalone.zip`）が必要です。
