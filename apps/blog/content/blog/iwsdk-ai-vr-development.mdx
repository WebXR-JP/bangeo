...
tags:
  ...
  - "spatial UI"
author: "BANGEO Team"
thumbnail: "/assets/tech/iwsdk-gallery-detail-xr.webp"
draft: true
---
...
物理演算・hand tracking・grab interaction・移動・spatial UIといったVR開発に必要な機能がすでに組み込まれていて、`npm create @iwsdk@latest` の1コマンドでプロジェクトが立ち上がります。さらに、Claude CodeやCodexなどのAIコーディングツールとの連携が最初から設計されており、プロンプトだけでVR体験を作り上げることもできます。

> ※ hand trackingはWebXR Hand Input Moduleの機能で、IWSDKではgrab interactionと組み合わせて使います。spatial UIは空間に配置するUIで、IWSDKではUIKitMLが該当します。
...
![IWSDKのデフォルトVRシーン - デスク、ロボット、WebXRロゴが配置されたオフィス空間](/assets/tech/iwsdk-vr-scene.webp)
...
![BANGEO WebXR Article Gallery - 記事サムネイルが半円形に配置されたVRギャラリー](/assets/tech/iwsdk-gallery-result.webp)

「Enter XR」ボタンを押すと、IWERエミュレーターによるVR視点に切り替わります。Meta Quest 3のコントローラーが表示され、ヘッドセットで見渡すように記事を閲覧できます。

![VRモードで表示したArticle Gallery - コントローラーで記事カードを操作できる](/assets/tech/iwsdk-gallery-xr.webp)
...
![VR空間内に表示された記事詳細パネル - ギャラリーの横に記事内容が表示されている](/assets/tech/iwsdk-gallery-detail.webp)

XRモードでも同様に、コントローラーで記事カードを選択すると横に詳細パネルが現れます。

![XRモードで表示された記事詳細パネル - コントローラーで操作可能](/assets/tech/iwsdk-gallery-detail-xr.webp)
...
| 物理演算 | Havok（WASM） | rigid body simulation、衝突判定 |
| ビルド | Vite | HMRによる即時リロード |
| XRエミュレーション | IWER | ヘッドセットなしでのXRセッション再現 |
| spatial UI | UIKitML | VR空間内のパネルUI構築 |
...
**IWER**（Immersive Web Emulation Runtime）はIWSDKに組み込まれたXRエミュレーターです。ヘッドセットの位置・回転、コントローラーのボタン入力、hand trackingのジェスチャーをすべてプログラムから操作できます。
...
エミュレーター経由で、ヘッドセットの移動やコントローラーのボタン操作、pinch gestureなどをプログラム的に実行します。AIがオブジェクトを掴んだりボタンを押したりして、インタラクションの動作を確認します。
...
- プロンプトからVRギャラリーやspatial UIが形になる
