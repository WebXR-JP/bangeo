...
1. **spatial mapping / scene capture** - 部屋の形状をスキャン
...
Meta Questの**Spatial Anchors**技術がベースになっています：

| ステップ | 処理内容 |
| --- | --- |
| 1. scene capture | Questのカメラ・センサーが部屋の壁、床、家具などの**feature points（featurePointCloud）**を検出 |
...
マルチプレイヤーARでの弾丸発射とhit testは、以下の流れで実現しています。
...
- **Spatial Anchors** は Quest の空間アンカー機能。WebXR では Anchors Module に対応
- **scene capture** は Quest の部屋形状取得機能。spatial mapping の一部
- **feature points** は WebXR の featurePointCloud プロパティで取得する特徴点
- **hit test** は WebXR Hit Test Module の機能
