...
import { type PlatformProfile, WEBXR_PLATFORMS } from "@/data/platforms";

export const metadata: Metadata = {
	title:
		"WebXRプラットフォーム｜Quest Browser・iOS Safari・visionOS・Chrome・Edge",
	description:
		"WebXRを実装・配信・体験するための主要ブラウザプラットフォーム（Meta Quest Browser、Wolvic、Safari on visionOS、Chrome、Edge、Samsung Internet）や代表サービスを整理しています。",
	alternates: { canonical: "/platforms" },
};

function PlatformRow({ platform }: { platform: PlatformProfile }) {
	return (
		<div className="group p-6 hover:bg-white transition-all duration-200">
...
function PlatformSection({
	...
	const platforms = WEBXR_PLATFORMS.filter((p) => p.category === category);
			...
			<div className="space-y-24">
				...
				<header className="text-center max-w-4xl mx-auto space-y-8">
					...
					<p className="text-xl md:text-2xl text-gray-500 font-medium leading-relaxed">
						WebXR
						の実装・配信・体験に使われる主要なブラウザプラットフォームとサービスをまとめています。
					</p>
				</header>

				<PlatformSection title="ブラウザプラットフォーム" category="browser" />
				<PlatformSection title="サービスプラットフォーム" category="service" />
			</div>
		</div>
	);
}
