...
2. 公開記事と作業ガイドを分ける
   ...
   - イベントウォッチや調査メモから記事化するときは、「何を回収したか」ではなく「読者が何を理解できるか」に変換して書く
...
イベント・発表Recap記事:
- タイトルに「回収メモ」「候補」「ウォッチ更新」のような編集作業語を入れない
- 冒頭で「この発表から読者が何を理解できるか」を先に書く
- 「BANGEO向け」「BANGEOでの評価」「置き場所」「重要度」「confidence」「source_type」「派生記事」などの運用判断は本文に入れない
- セッション動画やRecapは、羅列ではなく「WebXR / WebGPU / WebAR / Android XRとどう関係するか」で整理する
- 結論は、サイト内更新計画ではなく、読者が次に確認すべき技術論点で終える
